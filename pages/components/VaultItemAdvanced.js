...
const VaultItemAdvanced = ({ nft, type, action, conn }) => {
      ...
      try {
        if (type === "staked") {
          const sigs = await conn.getSignaturesForAddress(
            new PublicKey(nft?.mint?.toString()),
            {
              limit: 1,
            }
          );
        ...
        if (type === "staked") {
          if (whale === "male") {
            if (dateStaked) {
              ...
              const diffInSecBase = dayjs(new Date()).diff(
                dateStaked,
                "seconds"
              );
              ...
              if (dayjs(new Date()).diff(dateStaked, "seconds") > 604800) {
                ...
                const diffInSecTier1 = dayjs(new Date())
                  .subtract(1, "week")
                  .diff(dateStaked, "seconds");
              ...
              if (dayjs(new Date()).diff(dateStaked, "seconds") > 1209600) {
                ...
                const diffInSecTier2 = dayjs(new Date())
                  .subtract(2, "week")
                  .diff(dateStaked, "seconds");
              ...
              const diffInCoins =
                diffInCoinsBase + diffInCoinsTier1 + diffInCoinsTier2;
    ...
    <VaultItemContainer>
      ...
      <VaultItemDetailsContainer>
        <div>
          ...
          >
            {" "}
            Total $KRILL Earned: {estEarn.toFixed(3)}{" "}
          </p>{" "}
        </div>{" "}
        <div>
          {" "}
          <button onClick={() => action(nft)}>
            {" "}
